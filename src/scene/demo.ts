...
import { simplex2d } from "./shader/simplex2d";
const glsl = require('glslify')

class InstancedMouseEffect {
  constructor(target, opts = {}, newPosition = new THREE.Vector3(80, 80, 80), follower = null ){
    ...
    // Renderer up
    let rendering = new Rendering(target, false)
    rendering.renderer.shadowMap.enabled = true;
    rendering.renderer.shadowMap.type = THREE.PCFSoftShadowMap;
    rendering.camera.position.z = opts.cameraPosition.z
    rendering.camera.position.y = opts.cameraPosition.y
    rendering.camera.position.x = opts.cameraPosition.x
    ...
    let d1 = new THREE.DirectionalLight(0xffffff, 8)
    ...
    let geometry = new THREE.BoxGeometry(size, 10, size);
    geometry = new RoundedBox(size, size, size, 0.05, 1);
    ...
    let material = new THREE.MeshPhysicalMaterial(opts.meshArgs)
  ...
  moveCamera({x, y, z, v, zoom}) {
    let t1= gsap.timeline()
    const pos = new THREE.Vector3(this.rendering.camera.position)
    t1.to(this.rendering.camera.position, {
      x,
      y,
      z,
      duration: 0.5,
    ...
    console.log(this.lookPoint, v)
    t1.to(this.lookPoint, {
      x: v.x,
      y: v.y,
      z: v.z,
      duration: 1,
      ease: "power2.inOut",
      onUpdate: (v) => {
        this.rendering.camera.lookAt(this.lookPoint)
    ...
    t1.to(this.zoom, {
      ...
      ease: "power2.inOut",
      onUpdate: (v) => {
        this.rendering.camera.zoom = this.zoom.value
      }
    }, 1)
